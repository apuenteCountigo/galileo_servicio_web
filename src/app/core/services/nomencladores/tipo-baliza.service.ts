import { Injectable } from '@angular/core';
import {
  HateoasResourceOperation,
  HateoasResourceService,
  NgxHateoasClientConfigurationService,
  PagedResourceCollection,
} from '@lagoshny/ngx-hateoas-client';
import {
  PageParam,
  Sort,
} from '@lagoshny/ngx-hateoas-client/lib/model/declarations';
import { Observable, shareReplay } from 'rxjs';
import { environment } from 'src/environments/environment';
import { TipoBaliza } from '../../models/momencaldores.model';

@Injectable({
  providedIn: 'root',
})
export class TipoBalizaService extends HateoasResourceOperation<TipoBaliza> {
  constructor(
    public resourceService: HateoasResourceService,
    private hateoasConfig: NgxHateoasClientConfigurationService
  ) {
    super(TipoBaliza);
  }

  public getAll(
    params?: PageParam,
    sort?: Sort
  ): Observable<PagedResourceCollection<TipoBaliza>> {
    this.changeUrl();
    return this.resourceService
      .getPage(TipoBaliza, {
        pageParams: params,
        sort,
      })
      .pipe(shareReplay());
  }

  changeUrl() {
    this.hateoasConfig.configure({
      http: {
        rootUrl: environment.API_URL_TIPO_BALIZA,
      },
      cache: {
        enabled: false,
      },
    });
  }
}
