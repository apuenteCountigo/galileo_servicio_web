...
export class BalizaFormComponent implements OnInit, AfterViewInit, OnDestroy, AfterContentInit {
  ...
  obsModelosBalizas: BehaviorSubject<Array<ModeloBaliza>> = new BehaviorSubject<Array<ModeloBaliza>>([]);
    ...
    this._nomencladorModelosBalizas
      ...
      .subscribe({
        next: (modeloBaliza: PagedResourceCollection<ModeloBaliza>) => {
          ...
          // Establecer el valor una vez que los modelos se hayan cargado
          const modeloSeleccionado = this.listModelosBalizas.find(
            (s) => s.id === this.balizaToEdit?.modelo?.id
          );
          if (modeloSeleccionado) {
            this.formModalBaliza.controls['modelo'].setValue(modeloSeleccionado);
          }
